...
from dschat.utils.utils import print_rank_0,get_tensor_stats,RunningMoments,AdaptiveKLController,FixedKLController
...
class DeepSpeedPPOTrainer():
    ...
    def __init__(self, rlhf_engine, args):
        ...
        # if args.ppo.target is not None:
        self.kl_ctl = AdaptiveKLController(0.05, 6, 10000) #init_kl_coef 0.05 target 6 horizon 10000
        # else:
        #     self.kl_ctl = FixedKLController(args.ppo.init_kl_coef)

        self.mean_kl = None
        self.accumulated_exp_stats = []
        self.exp_stats = None
    ...
    def _generate_sequence(self, prompts, mask, step):
        ...
        if self.actor_model.module.config.model_type == "llama":
            ...
            kwargs = {'do_sample':True,'max_new_tokens':self.max_answer_seq_len,'eos_token_id':50260}
        ...
        if self.args.print_answers and (step % self.args.print_answers_interval
            ...
            print(
                f"--- prompt --> step={step}, rank={torch.distributed.get_rank()}, {self.tokenizer.batch_decode(prompts, skip_special_tokens=True)[0:2]}"
            )
            print(
                f"--- ans    --> step={step}, rank={torch.distributed.get_rank()}, {self.tokenizer.batch_decode(ans, skip_special_tokens=True)[:2]}"
            )
            print(
                f"--- ans length --> step={step}, rank={torch.distributed.get_rank()}, {valid_ans_len}"
            )
        out_seq = []
    ...
    def generate_experience(self, prompts, mask, step):
        ...
        # if self.args.clip_reward_value:
        reward_score = torch.clip(reward_score, -self.clip_reward_value, self.clip_reward_value)

        # store statistics of the initial rollout as reference
        stats = {}
        # scores_mean, scores_std = self.running_moments.update(reward_score)
        # stats["rollout_scores/mean"] = scores_mean.item()
        # stats["reward/std"] = scores_std.item()
        ...
        # actor ppl
        shift_label = seq[:, start+1:].contiguous()
        valid_length = (shift_label != self.tokenizer.pad_token_id).sum(dim=-1)
        shift_label[shift_label == self.tokenizer.pad_token_id] = self.ppl_loss_fct.ignore_index
        
        shift_logits = logits[:, start:-1, :].contiguous()
        ...
        ppl_value = torch.exp(ppl_value)
        ppl_value = ppl_value.cpu().tolist()

        stats["actor/ppl"] = sum(ppl_value) / len(ppl_value)
        # stats["actor/res_length"] = valid_length.float().mean()

        if torch.distributed.is_initialized():
            torch.distributed.all_reduce(mean_kl, torch.distributed.ReduceOp.AVG)
        
        stats["actor/sqrt_kl"] = torch.sqrt(mean_kl).item()
        stats["actor/kl_per_token"] = torch.sqrt(mean_kl_per_token).item()
        self.accumulated_exp_stats.append(stats)
        return {
            'prompts': prompts,
            'logprobs':logprobs,
    ...
    def train_rlhf(self, inputs):
        ...
        actor_loss, pg_clipfrac, ratio, approx_kl = self.actor_loss_fn(actor_log_prob[:, start:],
        ...
        value = self.critic_model.forward_value(**batch,
                                                ...
                                                use_cache=False)[:, :-1]
        critic_loss,vf_clipfrac = self.critic_loss_fn(value[:, start:], old_values[:,
                                                                       start:],
        ...
        n = action_mask[:, start:].sum()
        stats = {
            'losses/actor_loss':actor_loss.item(),
            'losses/critic_loss':critic_loss.item(),
            'values':get_tensor_stats(values[:, start:], action_mask[:, start:], n)['mean'],
            'returns':get_tensor_stats(returns, action_mask[:, start:], n)['mean'],
            'actor/advantages':get_tensor_stats(advantages, action_mask[:, start:], n)['mean']
        }
    ...
    def actor_loss_fn(self, logprobs, old_logprobs, advantages, mask):
        ...
        pg_clipfrac = torch.sum((pg_loss2 > pg_loss1).float() * mask) / mask.sum()
        return pg_loss, pg_clipfrac, ratio, approx_kl
    ...
    def critic_loss_fn(self, values, old_values, returns, mask):
        ...
        vf_clipfrac = torch.sum((vf_loss2 > vf_loss1).float() * mask) / mask.sum()
    ...
    def post_gen_callback(self):
        self.exp_stats = {k: sum([xs[k] for xs in self.accumulated_exp_stats])  
                        / len(self.accumulated_exp_stats) for k in self.accumulated_exp_stats[0]}
        self.exp_stats["kl_ctl_value"] = self.kl_ctl.value
        self.mean_kl = self.exp_stats["actor/sqrt_kl"] ** 2
        self.accumulated_exp_stats = []

    def post_backward_callback(self):
        self.kl_ctl.update(self.mean_kl,
                           n_steps=self.args.per_device_training_batch_size)
