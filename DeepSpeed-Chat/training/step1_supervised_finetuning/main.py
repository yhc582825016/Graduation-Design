...
import sys
sys.path.append('/workspace/ye/DeepSpeed-Chat')
import wandb
...
def main():
    ...
    train_dataset, eval_dataset = create_prompt_dataset(
        ...
        tokenizer,
        args.max_seq_len,
        sft_only_data_path=args.sft_only_data_path,reload=True)
    ...
    def evaluation(model, eval_dataloader):
        model.eval()
        losses = 0
        with tqdm(enumerate(eval_dataloader),total =len(eval_dataloader),disable=not args.global_rank!=0,desc='current evaling') as pbar:
            for step, batch in pbar:
                batch = to_device(batch, device)
                # print_rank_0(batch['input_ids'][0][0:200],args.global_rank)
                with torch.no_grad():
                    outputs = model(**batch)
                loss = outputs.loss
                losses += loss.float()
    ...
    for epoch in range(args.num_train_epochs):
        ...
        model.train()
        import time
        with tqdm(enumerate(train_dataloader),total=len(train_dataloader),disable=not args.global_rank!=0,desc='current Training') as pbar:
            for step, batch in pbar:
                glob_steps+=1
                start = time.time()
                batch = to_device(batch, device)
                ...
                if args.global_rank==0 and step % 10 ==0:
                        tokens = model.generate(**batch,max_new_tokens=512,do_sample=True,top_p=0.95,temperature=0.95,eos_token_id=2)
                        print(tokenizer.decode(tokens[0].tolist()[len(batch['input_ids'][0]):]))
                loss = outputs.loss
                if args.print_loss:
                    print(
                ...
                if glob_steps % int(len(train_dataloader)*args.eval_ratio) == 0:
                    print_rank_0(
