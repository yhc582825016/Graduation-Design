...
def main():
    ...
    def evaluation_reward(model, dataloader, eval_iters,global_rank):
        model.eval()
        correct_predictions = 0
        total_predictions = 0
        chosen_scores = 0.
        rejected_scores = 0.
        with tqdm(enumerate(dataloader),total=len(dataloader),desc='eval-rewad',disable=not global_rank!=0) as pbar:
            for _step, _batch in pbar:
    ...
    reward_score, reject_score, acc = evaluation_reward(
        rm_model, eval_dataloader, args.eval_iters,args.global_rank)
    print_rank_0(
    ...
    if args.global_rank==0:
        wandb.log({
                    ...
                    'eval/rejected_last_scores':reject_score,
                    'eval/acc':acc
                },total_micro_steps)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        rm_model.train()
        mean_loss = 0
        with tqdm(enumerate(train_dataloader),total=len(train_dataloader),disable=not args.global_rank==0) as pbar:
