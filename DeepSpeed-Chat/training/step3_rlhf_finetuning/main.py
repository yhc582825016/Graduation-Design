    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, (batch_prompt, batch_unsupervised) in enumerate(
            ...
            if exp_dataset is not None:
                trainer.post_gen_callback()
                if args.global_rank==0:
                    wandb.log(trainer.exp_stats, step) 
                inner_iter = 0
                ...
                for ppo_ep in range(args.ppo_epochs):
                    for i, (exp_data, unsup_data) in enumerate(
                        ...
                        actor_loss, critic_loss,stats = trainer.train_rlhf(exp_data)
                        ...
                        if unsupervised_training_enabled:
                            ...
                            stats.update({"losses/unsup_loss": unsup_loss.item()})
                        inner_iter += 1
                        if args.enable_ema:
                ...
                print_rank_0(f"Average reward score: {average_reward/inner_iter} | EMA reward score: {ema_reward_score.get()}",args.global_rank)
                print_rank_0("-------------------------------------------------------------------------------------",args.global_rank)

                if args.enable_wandb and torch.distributed.get_rank()==0:
                    print("exp_data['valid_ans_len']",exp_data['valid_ans_len'])
                    stats.update({
                            'avg_response_per_batch':exp_data['valid_ans_len'].float().mean().item(),
                            'reward': average_reward / inner_iter,
                            'EMA reward score': ema_reward_score.get()
                        })
