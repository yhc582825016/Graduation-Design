...
class CustomDataset_For_Rankloss(Dataset):
    ...
    def get_ids_with_principle(self,input_data):
        history,response = input_data
        
        principles_definition = ''
        collected_dimension = []
        for i in range(len(self.principles)):
            dimension = self.principles[i]['dimension']
            definition = self.principles[i]['definition']
            principles_definition += dimension + ":" + definition + "\n"
            collected_dimension.append(dimension)
        history = ''
        for i in input_data['context']:
            history+=i['role']+":"+i['text']+"\n"
        chosen,rejected = input_data['chosen']['text'],input_data['rejected']['text']
        prompt = CN_MULTI_PROMPT_V0001.format(history,chosen,rejected,self.principles_definition,",".join(collected_dimension))
        prompt_ids = self.tokenizer.encode(prompt, add_special_tokens=False)
        response_ids = self.tokenizer.encode(response, add_special_tokens=False)
        if len(prompt_ids) > self.max_src_len:
            prompt_ids = prompt_ids[-self.max_src_len:]
...
def create_rm_dataloder_multi(cfgs, train_dataset, eval_dataset1,eval_dataset2, tokenizer):
    ...
    # else:
    train_sampler = DistributedSampler(train_dataset)
    eval_sampler = DistributedSampler(eval_dataset1, shuffle=False)
    ...
    eval_dataloader1 = DataLoader(eval_dataset1,
                                 collate_fn=DataCollatorWithPadding(tokenizer, cfgs.dataset.max_seq_len),
                                 sampler=eval_sampler,
                                 batch_size=cfgs.train.per_device_eval_batch_size,
                                 pin_memory=True)
    eval_dataloader2 = DataLoader(eval_dataset2,
                                 collate_fn=DataCollatorWithPadding(tokenizer, cfgs.dataset.max_seq_len),
                                 sampler=eval_sampler,
                                 batch_size=cfgs.train.per_device_eval_batch_size,
                                 pin_memory=True)
    return train_dataloader, eval_dataloader1,eval_dataloader2
