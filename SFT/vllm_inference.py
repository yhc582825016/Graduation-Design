...
class LLMInference:
    ...
    def combine_prompt(self, sample):
        ...
        elif self.style == 'gov_data':
            # input_ = "Question:"+sample['question']+'\n\nAssistant:'
            input_="Question:"+sample['question']+'\n\nAnswer:'
        elif  self.style == 'gov_retrive_sft':
            if len(sample['question'])>2048:
                sample['question']= sample['question'][-2048:]
            input_="Question:"+sample['question']+'\n\nAnswer:'
        elif self.style == 'gov_mrc':
            input_ = "Question:"+sample['question']+'\n\nAssistant:'
        else:
...
def load_dataset_(data_path, data_type):
    if data_type == "sharegpt":
        ...
        data = load_json(data_path)
    elif data_type == "gov_data":
        data = load_dataset('/workspace/ye/DPO/Dataset/goverment_qa/finetune-que2res')
        return_data = []
        for que,res in zip(data['test']['question'],data['test']['response']):
            return_data.append({'question':que,'response':res})
        # random.shuffle(return_data)
        return return_data
    elif data_type=='gov_retrive_sft':
        data = load_json(data_path)
        return_data = []
        for i in data:
            return_data.append({'question':i['question'],
                                'response':i['answer']})
        return return_data
    elif data_type == "gov_mrc":
        data = load_json(data_path)
        return data
    else:
